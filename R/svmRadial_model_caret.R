#'
#'@title Support Vector Machine model Caret
...
#'@export
svmRadial_model_caret<-function(groundstations,
  ...
  # If this is regression, the most likely case is that the tree did not find a good split 
  # and used the average of the outcome as the predictor. That's fine but you cannot calculate 
  # R^2 since the variance of the predictions is zero.
  sumstat<-mod$results[which(mod$results$sigma==mod$bestTune$sigma & mod$results$C==mod$bestTune$C),]
  
  
 return(list("spatial"=pred,"model"=mod,"statistical_summary"=sumstat))
}
