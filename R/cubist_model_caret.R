...
#'@export
cubist_model_caret<-function(groundstations,
  ...
  sumstat<-mod$results[which(mod$results$committees==mod$bestTune$committees),]
  sumstat<-sumstat[which(sumstat$neighbors==mod$bestTune$neighbors),]
